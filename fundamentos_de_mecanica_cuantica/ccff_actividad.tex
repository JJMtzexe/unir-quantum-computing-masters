...
\begin{figure}[h]
    ...
    \caption{Modelo de cavidad para aproximar a un cuerpo negro ideal. La radiación proveniente de una fuente externa entra por un orificio pequeño y 
    sufre múltiples reflexiones en el interior de la cavidad. En cada reflexión, parte de la energía es absorbida por las paredes, de modo que la 
    radiación incidente pierde progresivamente “memoria” de su dirección e intensidad inicial. Cuando el sistema alcanza equilibrio térmico, la 
    cavidad reemite radiación electromagnética hacia el exterior, cuya distribución espectral depende principalmente de la temperatura de las 
    paredes y se aproxima al espectro ideal de un cuerpo negro. Fuente:\cite{planck1900}}
...
la ley dice que un objeto caliente emite radiación en todas las longitudes de onda y que la suma total de toda esa radiación depende fuertemente de la temperatura\cite{stefan1879}\\
...
emite con máxima intensidad se desplaza hacia longitudes de onda más cortas a medida que aumenta la temperatura, siguiendo una relación inversa\cite{wien1893}.\\

\textbf{La ley se expresa como:}
...
Según la teoría clásica, la luz era una perturbación periódica contínua (u onda electromagnética) que se propagaba en el espacio. Esta teoría predecía, por ejemplo, que en el efecto fotoeléctrico la energía de los electrones emitidos debería depender de la intensidad de la luz.
Sin embargo en el postulado del efecto fotoeléctrico ~\cite{einsteina1905_photoelectric} se demostró que la luz era de naturalez corpuscular (formada por cuantos) así como la existencia de los fotones. A través de la experimentación se demostró que los electrones saltaban de forma instantánea y solo si la luz superaba una frecuencia umbral específica, independientemente de cuán intensa fuera la luz.
\begin{equation}
    E_{\gamma} = h\nu = W_0 + E_{\text{cin}}
\end{equation}
Siendo que:
\begin{itemize}
\item $E_{\gamma}$ es la energía del fotón.
\item $h\nu$ es la constante de Plank ($h$) multiplicada por la frecuencia de la radiación ($\nu$).
\item $W_0$ es la energía de extracción.
\item $E_{\text{cin}}$ es la energía cinética máxima de los electrones emitidos.
\end{itemize}
Con la restricción de que $W_0$ debe ser mayor a $h\nu$ para que se produzca la emisión de electrones.

Es posible utilizar esta formulación para calcular la frecuencia umbral necesaria para que se produzca el efecto fotoeléctrico, una vez despejada la expresión:
\begin{equation}
