...
El modelo de \cite{rutherford1911} estableció que los electrones orbitan alrededor de un núcleo de carga positiva, pero según la electrodinámica clásica, un electrón en movimiento orbital debería emitir radiación de forma continua, perder energía y caer en espiral hacia el núcleo. \textbf{Esto, evidentemente, no ocurre.}

Bohr resolvió esta contradicción postulando que existen estados ``estacionarios'' en los que el electrón no radia, y que solo están permitidas aquellas órbitas cuyo momento angular sea un múltiplo entero de $\hbar$~\cite{bohr1913}. Con estos postulados obtuvo los niveles de energía del hidrógeno ($E_n = -13.6/n^2$ eV) y explicó su espectro de emisión con notable precisión.
...
donde $\ell$ describe la forma de la órbita (con valores $0, 1, \ldots, n-1$) y $m$ su orientación espacial (con valores $-\ell, \ldots, +\ell$). Sin embargo, la energía $E_n$ sigue dependiendo exclusivamente de $n$: los números $\ell$ y $m$ no aparecen en la expresión de la energía. Esto significa que órbitas con formas y orientaciones distintas tienen la misma energía, algo que no coincidía con las observaciones experimentales más precisas.

Además, al igual que en el modelo de Bohr, los valores permitidos de $\ell$ y $m$ se imponen mediante reglas de cuantización sin una justificación teórica de fondo. Son restricciones que funcionan, pero que nadie podía derivar desde una teoría más fundamental.

A pesar de los avances de ambos modelos, estos compartían limitaciones fundamentales:

\begin{itemize}
\item Los postulados de cuantización eran una imposición sobre la física clásica, no una consecuencia teórica.
...
Uno de los cuestionamientos más notables fue el del propio Rutherford, quien en una carta dirigida a Bohr señaló: ``¿cómo decide un electrón a qué frecuencia va a vibrar cuando pasa de un estado estacionario a otro? Pareciera que se debe asumir que el electrón sabe de antemano dónde se va a detener''~\cite{rutherford1913carta}. \textbf{Hacía falta, por tanto, una teoría completamente nueva.}

\subsection{De Broglie y su razonamiento de la dualidad onda-partícula}

En 1905, Albert Einstein demostró que la luz—hasta entonces considerada exclusivamente como una onda electromagnética—también se comportaba como partícula (el fotón $\gamma$ ), cuya energía viene dada por $E = h\nu$~\cite{einsteina1905_photoelectric}. En 1924, Louis de Broglie reflejó este razonamiento en su tesis doctoral: si las ondas pueden comportarse como partículas, ¿pueden las partículas comportarse como ondas?~\cite{debroglie1924}

De Broglie propuso que toda partícula con momento $p$ tiene asociada una longitud de onda:
...
Al despejar, se obtiene directamente:
...
que es precisamente la condición de cuantización del momento angular que Bohr había postulado sin justificación once años antes. Lo que en el modelo de Bohr era un postulado \textit{ad hoc}, en el razonamiento de de Broglie se convertía en una consecuencia natural de la naturaleza ondulatoria del electrón.

Sin embargo, este razonamiento aún tenía una limitación fundamental: la onda del electrón se trataba como unidimensional, envuelta alrededor de una trayectoria circular. 



Un electrón real existe en tres dimensiones, y su comportamiento ondulatorio debería describirse mediante una ecuación de ondas tridimensional, análoga a las ecuaciones de Maxwell para la luz. Como señala el material del curso, ``Schrödinger aprovecha la idea de De Broglie de asociar una onda a cada partícula, y desarrolla una ecuación de ondas que deben cumplir todas las partículas que pudiesen representarse mediante una función de onda''~\cite{tema2}.

\subsection{La ecuación de onda de Schrödinger}

De forma paralela al trabajo de Werner Heisenberg, quien en 1925 propuso abandonar las imágenes clásicas y trabajar directamente con matrices de magnitudes observables~\cite{heisenberg1925}, Erwin Schrödinger tomó un camino distinto: partiendo de la hipótesis de de Broglie, desarrolló una ecuación de ondas que toda partícula debía satisfacer~\cite{schrodinger1926}. El propio Schrödinger demostró un año más tarde que ambos enfoques---la mecánica matricial y la mecánica ondulatoria---eran matemáticamente equivalentes~\cite{tema2}. Como ejemplo, el momento angular en ambas representaciones se expresa como:
...
La ecuación de Schrödinger en su forma completa, dependiente del tiempo, es:
...
% SUGERENCIA DE IMAGEN: representación visual de la función de onda Ψ 
% como onda tridimensional alrededor del núcleo
...
Para el átomo de hidrógeno, el potencial en la ecuación~\ref{eq:schrodinger_ti} es el de Coulomb: $U(r) = -e^2/(4\pi\varepsilon_0 r)$. Este potencial depende únicamente de la distancia $r$ al núcleo, no de la dirección. Dado que el problema tiene simetría esférica, resulta natural resolverlo en coordenadas esféricas $(r, \theta, \phi)$, donde la función de onda puede separarse en una parte que depende de la distancia al núcleo y otra que depende de la orientación en el espacio:
...
Al exigir que esta función de onda sea físicamente válida---es decir, que no diverja al infinito, que sea continua y que la probabilidad total de encontrar al electrón en algún lugar sea igual a 1---la ecuación solo admite soluciones para ciertos valores discretos de tres parámetros. Estos parámetros son los números cuánticos, que emergen de la matemática sin necesidad de postularlos:

\begin{itemize}
\item $n = 1, 2, 3, \ldots$ (principal): determina la energía, $E_n = -13.6/n^2$ eV. Coincide con el resultado de Bohr, pero ahora como consecuencia de resolver la ecuación.
\item $\ell = 0, 1, \ldots, n-1$ (azimutal): determina el momento angular orbital $L = \sqrt{\ell(\ell+1)}\,\hbar$. Define las subcapas $s, p, d, f$.
\item $m_\ell = -\ell, \ldots, +\ell$ (magnético): determina la componente $z$ del momento angular, $L_z = m_\ell\hbar$. Explica el efecto Zeeman.
\end{itemize}

Los mismos números cuánticos que Bohr y Sommerfeld habían impuesto mediante reglas \textit{ad hoc} surgen aquí como consecuencias naturales de una sola ecuación diferencial.

% SUGERENCIA DE IMAGEN: formas de los orbitales s, p, d
...
Para el estado fundamental del hidrógeno ($n=1$, $\ell=0$), la densidad de probabilidad radial alcanza su máximo en $r = a_0$ (el radio de Bohr). El resultado de Bohr se recupera, pero con una interpretación distinta: $a_0$ no es la distancia a la que está el electrón, sino la distancia donde es más probable encontrarlo.
...
A pesar de representar un salto cualitativo respecto a la teoría cuántica antigua, el modelo de Schrödinger presenta limitaciones que el propio material del curso identifica~\cite{tema2}:
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\textwidth, height=0.2\textwidth]{images/espectroscopia_emision_h.jpg}
